...
이어서 생성형AI(Claude)에게 각 샘플의 악성 여부 판정을 요청한 뒤, AI가 근거로
제시한 API·섹션 이름이 실제 추출 결과와 일치하는지 대조했다.

분석 대상 6개 샘플과 SHA-256 해시는 `samples/SHA256SUMS.txt` 에 기록되어
있으며, 원본 실행 파일 자체는 저장소에 포함하지 않았다(사유는
`samples/README.md` 참고). 모든 분석은 파일을 바이트 단위로 읽거나 `pefile`
로 헤더만 파싱하는 정적 분석이며, 어떤 샘플도 실행하지 않았다.
...
`strings` 출력(사람이 읽을 수 있는 문자열 목록)만을 생성형AI에게 제공하고
"각 샘플의 악성 여부와 그 근거가 된 API 이름·섹션 이름을 함께 제시하라"고
요청했다. 이후 `pefile` 기반 전체 추출 결과(3~4장)와 AI의 주장을 항목별로
대조했다.
...
| packed_fixture.exe | 패킹된 실행 파일 특유의 `UPX0`/`UPX1` 형태 섹션명과 치환된 문자열이 함께 나타나 악성으로 의심 | 실제 섹션은 `.data` 1개(512바이트)뿐이며 UPX 계열 섹션명이 아님. 다만 Import DLL 0개(완전 공백)라는 결론은 실제와 일치 | **부분 불일치 (섹션명 근거는 틀렸으나, Import 공백에 기반한 결론 방향은 맞음)** |
| packed_upx_like.exe | `file` 판정상 UPX 패킹이 확인되며, `UPX0`/`UPX1` 섹션명과 `LoadLibraryA`·`GetProcAddress` 만 Import된 점이 패커의 전형적 특징으로 정적 탐지 회피 목적의 악성코드 의심 | 섹션명 `UPX0`/`UPX1`/`.rdata` 실제 존재. Import는 KERNEL32.dll의 `LoadLibraryA`, `GetProcAddress` 2개뿐(둘 다 의심 API 목록에 포함) | 일치 |
| suspicious_api.exe | `VirtualAlloc`/`WriteProcessMemory`/`CreateRemoteThread` 조합과 `http://training.invalid/checkin` 문자열로 보아, 프로세스 인젝션 후 `InternetOpenA` 로 외부 서버에 체크인하는 악성코드로 판단 | 위 3개 API는 실제로 Import에 존재(정확). 그러나 `InternetOpenA` 를 비롯한 네트워크 관련 API는 Import에 전혀 없음(ADVAPI32.dll, KERNEL32.dll의 8개 API만 존재) — URL은 문자열로만 존재 | **부분 불일치 (프로세스 인젝션 판단은 정확하나, 네트워크 API 사용 주장은 근거 없는 과잉 추론)** |

**AI가 잘못 지목한 항목 정리**

1. `normal_02.exe`: 완전히 틀린 판정 — 정상 데이터 테이블을 "인코딩된
   페이로드"로 오판. (엔트로피/무Import → 무조건 악성이라는 성급한 일반화)
2. `packed_fixture.exe`: 존재하지 않는 섹션 이름(`UPX0`/`UPX1`)을 근거로
   제시. 실제 섹션명은 `.data` 하나뿐이다. (패킹처럼 보이면 UPX라고
   단정하는 패턴 매칭 오류)
3. `suspicious_api.exe`: 실제로는 Import되지 않은 `InternetOpenA` 를
   호출한다고 주장. 문자열에 URL이 보인다는 이유만으로 네트워크 API 사용을
   추정한 과잉 추론이다.
...
생성형AI 검증 결과, 6건 중 2건은 AI의 판정·근거가 정확했고, 1건은 완전히
틀렸으며(`normal_02.exe`), 2건은 결론은 맞았으나 근거의 일부(존재하지
않는 섹션명, 존재하지 않는 API)가 사실과 달랐다. 이는 생성형AI의 결론만
받아들이지 않고 **원본 추출 결과와 반드시 대조해야 하는 이유**를 잘
보여준다 — 특히 AI는 "그럴듯한 패턴"(패킹된 것처럼 보이면 UPX 섹션명을
가정, URL 문자열이 보이면 네트워크 API를 가정)에 기반해 실제로 존재하지
않는 세부 근거를 자신 있게 만들어내는 경향이 있었다.
